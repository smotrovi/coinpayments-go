...
func TestRates(t *testing.T) {
	...
	rates, _, err := client.Rates.Show(&RateParams{Short: 0})
...
func TestRatesShort(t *testing.T) {
	...
	client := NewClient("", "", httpClient)
	rates, _, err := client.Rates.Show(&RateParams{Short: 1})
	expectedMap := map[string]RateInfo{}
	expectedMap["BTC"] = RateInfo{
		IsFiat:         0,
		RateBTC:        "1.000000000000000000000000",
...
func TestRatesWithAccepted(t *testing.T) {
	httpClient, mux, server := testServer()
	defer server.Close()
	...
	client := NewClient("", "", httpClient)
	rates, _, err := client.Rates.ShowWithAccepted(&RateParams{Short: 0})
	expectedMap := map[string]RateInfoWithAccepted{}
	expectedMap["BTC"] = RateInfoWithAccepted{
		IsFiat:         0,
		RateBTC:        "1.000000000000000000000000",
		LastUpdate:     "1375473661",
		TransactionFee: "0.00010000",
	...
	expectedMap["LTC"] = RateInfoWithAccepted{
		IsFiat:         0,
		RateBTC:        "0.003598757777777800000000",
		LastUpdate:     "1487464863",
		TransactionFee: "0.00100000",
	...
	expected := RateResponseWithAccepted{
		Error:  "ok",
		Result: expectedMap,
	}
	assert.Nil(t, err)
...
func TestRatesWithAcceptedShort(t *testing.T) {
	httpClient, mux, server := testServer()
	defer server.Close()
	...
	client := NewClient("", "", httpClient)
	rates, _, err := client.Rates.ShowWithAccepted(&RateParams{Short: 1})
	expectedMap := map[string]RateInfoWithAccepted{}
	expectedMap["BTC"] = RateInfoWithAccepted{
		IsFiat:         0,
		RateBTC:        "1.000000000000000000000000",
